...
#
# Usage:
#   iex (irm https://raw.githubusercontent.com/leox8136/hermes-agent/current-ops/scripts/install.ps1)
#
...
$RepoUrlSsh = "git@github.com:leox8136/hermes-agent.git"
$RepoUrlHttps = "https://github.com/leox8136/hermes-agent.git"
$PythonVersion = "3.11"
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/leox8136/hermes-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/leox8136/hermes-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/leox8136/hermes-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
function Write-BootstrapMarker {
    ...
    $pinnedBranch = $Branch
    if (-not $pinnedBranch) {
        $pinnedBranch = "current-ops"  # install.ps1's own default for -Branch
    }
...
function Install-Desktop {
    ...
    if (-not $env:GITHUB_REF_NAME) {
        $env:GITHUB_REF_NAME = if ($Branch) { $Branch } else { "current-ops" }
    }
    if ($env:GITHUB_SHA) {
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/leox8136/hermes-agent/current-ops/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
