...
def _verify_fleet_after_update(restart, *, _pre_update_plan, _windows_gateway_resume, node_failures, update_complete):
    ...
    from hermes_cli.update_handoff import defer_gateway_update_handoff, gateway_handoff_fleet_errors
    if getattr(restart, "self_restart_pending_pids", None) and defer_gateway_update_handoff(
        _fleet_snapshot, update_complete=update_complete and not restart.incomplete,
    ):
        print("\nGateway restart accepted; final version verification will continue in the successor gateway.")
