...
def _refresh_bootstrap_cache_scripts(branch: str = DEFAULT_UPDATE_BRANCH) -> None:
    ...
    with _best_effort('Could not refresh bootstrap-cache scripts after update: %s'):
        ...
        if not cache_dir.is_dir():
            return
        safe_ref = re.sub(r"[^A-Za-z0-9._-]", "_", str(branch or DEFAULT_UPDATE_BRANCH))  # install_script.rs::sanitize_ref()
        ...
        for kind, src_name in (("ps1", "install.ps1"), ("sh", "install.sh")):
            ...
            for prefix in ("install", "install-leox8136"):
                cached = cache_dir / f"{prefix}-{safe_ref}.{kind}"
