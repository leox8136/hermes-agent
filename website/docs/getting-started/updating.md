...
Messaging `/update` checkpoints its target commit, runtime identities and receipt after apply/maintenance and before restarting gateways. A systemd restart can kill the updater along with the old gateway even though the updater started a separate session. The new gateway resumes verification from `.update_handoff.json`: only when every expected gateway reports the target commit does it finish the receipt, clear `fleet_restart_pending` and its matching host-wide restart obligation, and publish the result. Missing or unknown identities remain pending; a completed code update alone is not a successful fleet update. If the gateway accepts a self-restart that waits for the updater to exit, the receipt stays open and the same successor verification owns the final result. Recovery does not restart services or change systemd's `KillMode`. If the inventory includes non-gateway runtimes that the successor cannot verify, it preserves the obligation for operator inspection.

This recovery requires a checkpoint written by the updated updater. Older interrupted runs with only a `fleet_restart_pending` breadcrumb cannot be automatically certified from that marker alone.
